...
namespace ArtCollective.Data
{
    ...
    {
        private const string ConnectionString = @"Server=SIMEON\SQLEXPRESS;Database=ArtCollectiveDb;Trusted_Connection=True;Encrypt=False";

        public DbSet<Artist> Artists { get; set; }
        public DbSet<Group> Groups { get; set; }
        public DbSet<Artwork> Artworks { get; set; }
        public DbSet<Feedback> Feedbacks { get; set; }
        public DbSet<ArtistGroup> ArtistsGroups { get; set; }
        ...
        {
            modelBuilder.Entity<ArtistGroup>().HasKey(x => new { x.GroupId, x.ArtistId });
            modelBuilder.Entity<ArtistGroup>().HasOne(x => x.Artist).WithMany(x => x.ArtistsGroups).HasForeignKey(x => x.ArtistId);
            modelBuilder.Entity<ArtistGroup>().HasOne(x => x.Group).WithMany(x => x.ArtistsGroups).HasForeignKey(x => x.GroupId);

            modelBuilder.Entity<Artwork>().HasOne(x => x.Artist).WithMany(x => x.Artworks).HasForeignKey(x => x.ArtistId);

            modelBuilder.Entity<Feedback>().HasOne(x => x.Artist).WithMany(x => x.Feedbacks).HasForeignKey(x => x.ArtistId);
            modelBuilder.Entity<Feedback>().HasOne(x => x.Group).WithMany(x => x.Feedbacks).HasForeignKey(x => x.GroupId);

            modelBuilder.Entity<Collaboration>().HasKey(x => new { x.ArtistOneId, x.ArtistTwoId });
            modelBuilder.Entity<Collaboration>().HasOne(x => x.ArtistOne).WithMany(x => x.CollaborationWithTwo).HasForeignKey(x => x.ArtistOneId).OnDelete(DeleteBehavior.NoAction);
            modelBuilder.Entity<Collaboration>().HasOne(x => x.ArtistTwo).WithMany(x => x.CollaborationWithOne).HasForeignKey(x => x.ArtistTwoId).OnDelete(DeleteBehavior.NoAction);
